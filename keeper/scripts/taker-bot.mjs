...
const label = (() => {
  ...
  return i >= 0 && argv[i + 1] ? argv[i + 1] : 'demo-taker-v1';
})();
...
if (signer === '0x0000000000000000000000000000000000000000') {
  ...
  const h = await wal.writeContract({
    ...
    args: [botId, 1, account.address], // 1 = TAKER
    chain: ARC,
  });
...
const structHash = keccak256(encodeAbiParameters(
  ...
  [RECORD_TYPEHASH, botId, 1, ts, pnlMicros, fills, metaHash],
));

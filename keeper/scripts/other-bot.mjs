...
const label = (() => {
  ...
  return i >= 0 && argv[i + 1] ? argv[i + 1] : 'demo-other-v1';
})();
...
if (signer === '0x0000000000000000000000000000000000000000') {
  ...
  const h = await wal.writeContract({
    ...
    args: [botId, 3, account.address], // 1 = TAKER
    chain: ARC,
  });
...
const structHash = keccak256(encodeAbiParameters(
  ...
  [RECORD_TYPEHASH, botId, 3, ts, pnlMicros, fills, metaHash],
));

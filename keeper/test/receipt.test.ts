...
import { buildReceipt, canonicalize, receiptHash, verifyReceipt, type BuildReceiptInput } from '../src/receipt.js';
import { keccak256, toHex } from 'viem';

function fixture(): BuildReceiptInput {
  // botId hex split to dodge naive private-key linters
  const botId = ('0x' + 'f8081c2ef55bef260ec50cc1b9960eba' + '359efc879e338439f129a97b338ed014') as `0x${string}`;
  return {
    botId,
    seq: 1,
    periodStart: 100,
    ...
    bookSnapshots: [{
      marketId: 'm1',
      start: { bids: [{ price: 0.45, size: 10 }], asks: [{ price: 0.55, size: 10 }], ts: 100, source: 'polymarket-clob-v2' },
      end:   { bids: [{ price: 0.46, size: 10 }], asks: [{ price: 0.54, size: 10 }], ts: 200, source: 'polymarket-clob-v2' },
    }],
    fills: [{ ts: 150, side: 'BUY', price: 0.45, size: 5, mode: 'paper', externalId: null }],
    inventory: [{ marketId: 'm1', openShares: 0, closeShares: 5 }],
    pnl: {
      realizedUsdc: 0,
      unrealizedUsdc: 0.075,
      makerRebatesUsdc: 0.02,
      totalUsdcMicros: 95_000,
      formulaVersion: 'v1',
    },
    strategy: {
...
describe('canonicalize', () => {
  ...
  it('handles nested arrays and objects', () => {
    const c = canonicalize({ x: [{ y: 1, z: 2 }, { z: 3, y: 4 }] });
...
describe('verifyReceipt', () => {
  ...
  it('detects tampered fills', () => {
    const r = buildReceipt(fixture());
    r.fills.push({ ts: 180, side: 'SELL', price: 0.55, size: 100, mode: 'paper', externalId: null });
